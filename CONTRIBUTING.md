...
This repo packages [Uptime Kuma](https://github.com/louislam/uptime-kuma) for StartOS.
...
Uptime Kuma runs the upstream `louislam/uptime-kuma` image straight from Docker Hub — there is no fork, Dockerfile, or submodule. To track a new upstream release:

1. Bump `dockerTag` in `startos/manifest/index.ts` to `louislam/uptime-kuma:<new version>`.
