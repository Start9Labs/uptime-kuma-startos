...
- Switch the root CA install in `startos/main.ts` to `sdk.getRootCa(effects)`
  ...
  2.0.9). The install mints a certificate it has no use for and takes the root
  as element `[2]` of the returned fullchain, because that was the only way to
  reach the root. `getRootCa` returns it directly, so both the hostname
  argument and the chain indexing go away.
